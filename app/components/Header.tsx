    ...
    <>
      ...
      <header className="w-full fixed top-0 left-0 z-50 bg-white shadow-md border-b border-gray-200">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-4">
          ...
          <nav className="hidden md:flex gap-10 text-lg font-medium items-center">
            {menuItems.map((item) => {
              if (item.label === "Services") {
                return <ServicesDropdown key="services-dd" />;
              }

              if (item.label === "Partners") {
                return <PartnersDropdown key="partners-dd" />;
              }
          ...
          {/* DESKTOP CTA */}
          <button
            className="hidden md:block bg-primary text-white px-5 py-2.5 rounded-md font-semibold shadow-sm
      ...
      {/* MOBILE SIDE PANEL */}
      <div
        className={`fixed top-0 right-0 h-full w-[70%] max-w-xs bg-white shadow-2xl border-l border-gray-200 
